...
function createPackagedPluginLayout() {
  const sandboxDir = mkdtempSync(join(tmpdir(), "easycode-skill-layout-"))
...
function createCopiedPluginLayout() {
  const sandboxDir = mkdtempSync(join(tmpdir(), "easycode-copied-skill-layout-"))
...
describe("resolvePluginSkillPath", () => {
  it("returns the packaged plugin ../src/skills path when directory exists", () => {
    ...
    try {
      expect(skillPath.startsWith(sandboxDir)).toBeTrue()
