...
function buildLargeReplaceFixture(count: number): { source: string; output: string; json: string } {
  ...
  for (let i = 0; i < count; i++) {
    ...
    matches.push(`{"file":"many.ts","range":{"start":{"line":${i},"column":${exprStart}}},"text":"a + b","replacement":"b + a","replacementOffsets":{"start":${offset + exprStart},"end":${offset + exprStart + 5}}}`)
...
describe("ast_grep_search", () => {
  it("uses tool context root when paths are omitted", async () => {
    ...
    const result = String(await (ast_grep_search.execute as any)(
      {
...
describe("ast_grep_replace", () => {
  it("updates files when dryRun is false", async () => {
    ...
    const result = String(await (ast_grep_replace.execute as any)(
      {
        pattern: "return $A + $B",
        rewrite: "return $B + $A",
        lang: "typescript",
  ...
  it("applies relative match paths against the execution root", async () => {
    const dir = mkdtempSync(join(tmpdir(), "ast-relative-"))
    tempDirs.push(dir)
    const filePath = join(dir, "relative.ts")
    writeFileSync(filePath, "const sum = a + b\n", "utf-8")
    writeMockSg(dir, '[{"file":"relative.ts","range":{"start":{"line":0,"column":0}},"text":"a + b","replacement":"b + a","replacementOffsets":{"start":12,"end":17}}]')
    ...
    expect(result).toContain("were made")
    expect(readFileSync(filePath, "utf-8")).toContain("b + a")
  })
