...
function formatSearch(result: ReturnType<typeof parseSgJson>): string {
  ...
  const visibleMatches = result.matches.slice(0, MAX_FORMATTED_MATCHES)
  const lines = visibleMatches.map((match, index) => `${index + 1}. ${match.file}:${match.line}:${match.column}\n${match.text}`)
...
export const ast_grep_search: ToolDefinition = tool({
  description: "Search code patterns using AST-aware matching. Supports complete AST-node patterns with $VAR and $$$ placeholders.",
  args: {
  ...
  async execute(args, context) {
    try {
      ...
      const cmdArgs = buildRunArgs(args, cwd)
      const { stdout, stderr, code, timedOut } = await runSg(cmdArgs, cwd)
