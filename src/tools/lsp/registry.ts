...
export const SERVERS: Record<string, LspServerDef> = {
  typescript: { id: "typescript", cmd: ["typescript-language-server", "--stdio"], exts: [".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".mts", ".cts"], hint: "npm install -g typescript-language-server typescript", markers: ["tsconfig.json", "jsconfig.json", "package.json"], priority: 40 },
  deno: { id: "deno", cmd: ["deno", "lsp"], exts: [".ts", ".tsx", ".js", ".jsx", ".mjs"], hint: "Install Deno from https://deno.land", markers: ["deno.json", "deno.jsonc"], priority: 60 },
...
export function isCommandAvailable(cmd: string, root?: string): boolean {
  if (root) {
    const localBase = join(root, "node_modules", ".bin", cmd)
    const localExts = process.platform === "win32" ? ["", ".exe", ".cmd", ".bat"] : [""]
...
export function selectServerForFile(
  ...
  isAvailable = (cmd: string) => isCommandAvailable(cmd, root),
): LspServerDef | null {
  const candidates = Object.values(SERVERS)
    ...
    .map((server) => {
      ...
      const hasLocalBinary = existsSync(localBase) || existsSync(localBase + ".cmd") || existsSync(localBase + ".exe")
      if (hasLocalBinary) score += 40
