...
class LspManager {
  private pool = new Map<string, ManagedClient>()
  private pending = new Map<string, Promise<LspClient>>()
  private timer: ReturnType<typeof setInterval> | null = null
  private static readonly instanceValue = new LspManager()

  static get(): LspManager {
    return LspManager.instanceValue
  }
  ...
  async acquire(root: string, id: string, cmd: string[]): Promise<LspClient> {
    ...
    const promise = (async () => {
      ...
      const client = new LspClient(root, cmd)
      await client.start()
...
async function executeLspTool<T>(
  filePath: string,
  fn: (client: LspClient, root: string, absPath: string) => Promise<T>,
): Promise<LspResult<T>> {
  ...
  try {
    const value = await fn(client, root, absPath)
